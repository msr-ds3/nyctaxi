...
data <- taxi_clean %>% group_by(pickup_neighborhood, pickup_hour) %>% summarize(summary = n()/7)

plot_data <- tidy(nyc_neighborhoods, region="neighborhood") %>% 
  left_join(., data, by=c("id"="pickup_neighborhood"))

p <- ggmap(nyc_map) + 
  geom_polygon(data=plot_data, aes(x=long, y=lat, group=group, fill=summary, frame=pickup_hour), color="black", size = 0.25, alpha=0.5) + 
