...
nyc_map <- get_map(location = c(lon = -73.95, lat = 40.71), 
                   maptype = "terrain", 
                   zoom = 11, color="bw")
...
make_ggmap <- function(df, map, spdf, join_by, 
                       boundary_color="black", 
                       boundary_thickness=0.25, fill_alpha=0.5, 
                       pal = "Spectral", transform = "log10", 
                       breaks =c(1 %o% 10^(0:5)), na_color = "#808080", 
                       title=waiver())
{
  plot_data <- get_plot_df(spdf, df, join_by)
  ggmap(map) + 
    geom_polygon(data=plot_data, 
                 aes(x=long, y=lat, group=group, fill=summary), 
                 color=boundary_color, 
                 size = boundary_thickness, 
                 alpha=fill_alpha) + 
...
get_map_by_range <- function(df, 
                             begin=0, 
                             end=24, 
                             group_by, 
                             filter_by ="pickup_hour")
  {
  data <- df %>% 
    filter_(paste(filter_by, ">=", begin, "&", filter_by, "<", end)) %>%
    group_by_(group_by) %>%
    summarize(summary = n()/(7*(end-begin)))
  make_ggmap(data, 
             nyc_map, nyc_neighborhoods,
             join_by = c("id"=group_by),
             title=begin)
}

get_map_by_driver <- function(df, driver, shift = NULL)
  {
  ...
  if (!is.null(shift))
  {
    ...
    data <- data %>% 
      ...
      summarize(summary = n())
  make_ggmap(data, nyc_map, nyc_neighborhoods, 
             join_by = c("id"="neighborhood"), 
             transform = "identity", 
             breaks = seq(1, max(data$summary), round(max(data$summary)/5)))
}
