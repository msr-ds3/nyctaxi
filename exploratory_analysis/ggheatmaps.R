...
data <- taxi_clean %>% group_by(pickup_neighborhood, hour) %>% summarize(summary = n())
make_ggmap(data, nyc_map, nyc_neighborhoods, join_by = c("id"="pickup_neighborhood"))

for (i in 0:23)
{
  get_map_by_range(taxi_clean, begin = i, end = i+1, group_by = "dropoff_neighborhood")
  ggsave(paste(i, ".png"))
}
